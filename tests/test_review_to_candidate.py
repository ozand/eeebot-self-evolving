...
def test_inspect_pass_streak_generates_concrete_followup_candidate(tmp_path: Path):
    ...
    assert any(item.get('task_id') == 'materialize-pass-streak-improvement' for item in (current.get('generated_candidates') or []))
    decision = current.get('feedback_decision') or {}
    assert decision.get('mode') == 'promote_review_followup'
